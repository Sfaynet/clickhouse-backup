...
func (b *Backuper) restoreDataRegular(ctx context.Context, backupName string, backupMetadata metadata.BackupMetadata, tablePattern string, tablesForRestore ListOfTables, diskMap, diskTypes map[string]string, disks []clickhouse.Disk, skipProjections []string, replicatedCopyToDetached bool, existingTablesSnapshot []clickhouse.Table) error {
	...
	// Prefetch remote backup metadata chain once before parallel goroutines start.
	// Without this, each goroutine calls BackupList (S3 LIST + global lock + disk I/O)
	// independently for the same RequiredBackup, producing O(N_tables) redundant remote calls.
	if backupMetadata.RequiredBackup != "" && b.dst != nil {
		if prefetchErr := b.prefetchRemoteBackupMetadata(ctx, backupMetadata.RequiredBackup); prefetchErr != nil {
			log.Warn().Err(prefetchErr).Msg("restoreDataRegular: prefetchRemoteBackupMetadata failed, will fall back to per-goroutine fetching")
		}
	}
...
func (b *Backuper) findObjectDiskPartRecursive(ctx context.Context, backup metadata.BackupMetadata, table metadata.TableMetadata, part metadata.Part, diskName string, logger zerolog.Logger) (string, string, error) {
	...
	if err != nil {
		return "", "", errors.WithMessage(err, "ReadBackupMetadataRemoteCached")
	}
